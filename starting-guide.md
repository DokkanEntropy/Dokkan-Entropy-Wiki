...
[**DISCLAIMER Dokkan Entropy is currently in beta and limited to beta-testers. If you'd like to become one simply join the discord and become verified.**](https://discord.gg/3j9PpUgPFJ)

Firstly you'll need to install the [**IPA**](https://discord.com/channels/961349987059531928/962947129935024128/994886846146428988) onto your device. There are numerous way's to do this. Some sideloading services are listed below.
...
Upon completing the Discord Authorization. You'll need to create a new bot account. Currently Entropy uses this bot account to grab the assets from the offical servers. When the patching portion of Entropy is complete we will remove the dependency on the offical servers.
...
The server will be running when you're greeted to an ip address and port number listed.
...
Enter `http://server_ip` into the hostname field. Replace `server_ip` with the ip of the machine running the Dokkan Entropy Server. You can also use `http://your.domain.com` if you have a registered domain resolving to your machine or vps. If you'd like to support https then on Entropy server hit `[6] Edit Settings`
